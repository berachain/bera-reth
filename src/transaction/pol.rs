...
/// Gas limit encoded in every Prague1 PoL system transaction.
///
/// This is a consensus constant, distinct from the containing block's gas limit. Changing it
/// requires an explicit network fork so old and new clients agree on the synthetic transaction
/// hash.
...
pub fn create_pol_transaction(
    ...
    let pol_tx = PoLTx {
        ...
        nonce,
        gas_limit: POL_TX_GAS_LIMIT,
        // Gas price is set to the base fee for RPC compatibility.
...
mod tests {
    ...
    #[test]
    fn test_pol_transaction_validation_rejects_block_gas_limit() {
        ...
        let BerachainTxEnvelope::Berachain(pol_tx) = pol_tx_envelope else {
            panic!("Expected PoL transaction");
        };

        let mut malformed_pol_tx = pol_tx.clone_inner();
        malformed_pol_tx.gas_limit = BLOCK_GAS_LIMIT;
        let malformed_pol_tx = Sealed::new(malformed_pol_tx);

        let validation_result =
            validate_pol_transaction(&malformed_pol_tx, chain_spec, pubkey, block_number, base_fee);

        assert!(
            validation_result.is_err(),
            "PoL transaction using the containing block gas limit must fail validation"
        );
        assert!(validation_result.unwrap_err().to_string().contains("hash mismatch"));
    }
